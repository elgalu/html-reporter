...
const assertCustomGuiSectionGroup = (sectionName, group, groupIndex) => {
    if (!_.isPlainObject(group)) {
        throw new Error(`customGui[${sectionName}][${groupIndex}] option must be an object, but got ${typeof group}`);
    }
};

const assertCustomGuiSection = (section, sectionName) => {
    if (!_.isArray(section)) {
        throw new Error(`customGui[${sectionName}] option must be an array, but got ${typeof section}`);
    }
    section.forEach((group, groupIndex) => assertCustomGuiSectionGroup(sectionName, group, groupIndex));
};

const assertCustomGui = (customGui) => {
    if (!_.isPlainObject(customGui)) {
        throw new Error(`"customGui" option must be plain object, but got ${typeof customGui}`);
    }
    for (const sectionName in customGui) {
        assertCustomGuiSection(customGui[sectionName], sectionName);
    }
};
