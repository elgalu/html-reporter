...
exports.start = ({paths, hermione, guiApi, configs}) => {
    ...
    server.get('/init', (req, res) => {
        for (const sectionName in pluginConfig.customGui) {
            pluginConfig.customGui[sectionName].forEach((ctx) =>
                ctx.initialize && ctx.initialize({hermione, ctx}));
        }
    ...
    server.post('/run_custom_gui_action', ({body: payload}, res) => {
        const {sectionName, groupIndex, controlIndex} = payload;

        if (isUndefined(sectionName) || isUndefined(groupIndex) || isUndefined(controlIndex)) {
            res.sendStatus(400); // bad request
            return;
        }

        const ctx = pluginConfig.customGui[sectionName][groupIndex];
        const control = ctx.controls[controlIndex];

        if (ctx && ctx.action) {
            ctx.action({hermione, control, ctx});
        }
